...
const getEmployment = async (id) => {
    const employmentData = await Employment.findOne({
        ...
        attributes: ['EMPLOYMENT_ID', 'EMPLOYMENT_CODE', 'EMPLOYMENT_STATUS', 'HIRE_DATE_FOR_WORKING', 'WORKERS_COMP_CODE', 'TERMINATION_DATE', 'REHIRE_DATE_FOR_WORKING', 'LAST_REVIEW_DATE', 'PERSONAL_ID']
    }).catch(err => {
        console.log(err);
